shared_examples "Create Accounts: Create New Editor User" do
  it "Create New Editor User" do
  end
  ...
  it "Create a new user" do
    ...
    $browser.find_element(:xpath, "(.//input[@name='role'])[3]").click
  ...
  it "Confirm values for new user" do
    expect(browser_page_text).to include("A new account for Eddie Editor was successfully created.")
    $browser.find_element(:link_text, "EddieEditor@mydomain.edu")
    expect($browser.find_element(:xpath, ".//tr[2]/td[1]").text).to eq("EddieEditor@mydomain.edu")
    expect($browser.find_element(:xpath, ".//tr[2]/td[2]").text).to eq("Eddie")
    expect($browser.find_element(:xpath, ".//tr[2]/td[3]").text).to eq("Editor")
    expect($browser.find_element(:xpath, ".//tr[2]/td[4]").text).to eq("ACTIVE")
    expect($browser.find_element(:xpath, ".//tr[2]/td[5]").text).to eq("Editor")
    expect($browser.find_element(:xpath, ".//tr[2]/td[6]").text).to eq("0")
  end
